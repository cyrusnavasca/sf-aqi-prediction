...
class IngestAQIData(DataIngestionStrategy):
    def get_data(self) -> pd.DataFrame:
        data = pd.read_csv("../data/raw/sf_aqi_historic.csv")
        return data

class IngestWeatherData(DataIngestionStrategy):
    def get_data(self) -> pd.DataFrame:
        data = pd.read_csv("../data/raw/sf_weather_historic.csv")
        return data

class IngestMapsData(DataIngestionStrategy):
    def get_data(self) -> pd.DataFrame:
        pass

class IngestCurrentData:
    # Hard-coded values for San Francisco
    zipcode = "94134"
